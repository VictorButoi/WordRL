...
SCREEN_WIDTH = NUM_COLUMNS*BOX_SIZE + (NUM_COLUMNS - 1)*BOX_SPACING + SCREEN_SPACING
...
def draw_board(board, 
    ...
    for row in range(height):
        for col in range(width):
            ...
            pygame.draw.rect(screen, colors[row], [x, y, box_size, box_size], do_fill)
            place_letter(x, y, col, box_size)

x_offset = (SCREEN_WIDTH - (NUM_COLUMNS*BOX_SIZE + (NUM_COLUMNS - 1)*BOX_SPACING))/2
...
while running:
    timer.tick(fps)
    screen.fill(black)
    draw_board(board, sox=x_offset, soy=y_offset, box_size=BOX_SIZE, x_space=BOX_SPACING, y_space=BOX_SPACING)
