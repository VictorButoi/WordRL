...
class WordleEnv_v2(gym.Env):
    ...
    def step(self, action):
        ...
        # update the state
        state[0] -= 1
        for i, c in enumerate(action):  # iterate over letters in the guess
            ...
            if self.goal_word[i] == c:
                # GREEN LETTER: c is at position i, all other chars are not at position i
                ...
                # YELLOW LETTER: c is not at position i, but could be at other positions
                state[offset + 3 * i:offset + 3 * i + 3] = [1, 0, 0]
            else:
                # GREY LETTER: c is not in the goal word
